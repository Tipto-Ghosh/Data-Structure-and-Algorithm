...
class Solution {
public:
    void solve(int currIndex, int n , int sum , vector<int> &arr , int target , vector<int> &sub , set<vector<int>> &Set) {
       if(sum == target) {
        Set.insert(sub);
        return;
       }
       ...
       for(int i = currIndex; i < n; i++) {
          // avoid dublicates of i
          if(i > currIndex && arr[i] == arr[i-1]) {
            continue;
          }
          sub.push_back(arr[i]);
          solve(i+1 , n , sum + arr[i] , arr , target , sub , Set); 
          sub.pop_back();
       } 
    }
    vector<vector<int>> combinationSum2(vector<int>& arr, int target) {
       ...
       solve(0 , n , 0 , arr , target , sub , Set);
