...
class Solution {
    vector<string> bellowTen = {"","One","Two","Three","Four","Five","Six","Seven","Eight","Nine"};
    vector<string> bellowTwenty = {"","Ten","Eleven","Twelve","Thirteen","Fourteen","Fifteen","Sixteen","Seventeen","Eighteen","Nineteen"};
    vector<string> bellowHundred = {"","Ten","Twenty","Thirty","Forty","Fifty","Sixty","Seventy","Eighty","Ninety"};

public:
    string numberToWords(int num) {
        if(num == 0) return "Zero";
        if(num < 10) {
            return bellowTen[num];
        }
        if(num < 20) {
            return bellowTwenty[num - 10];
        }
        if(num < 100) {
            return bellowHundred[num / 10] + (num % 10 != 0 ? " " + bellowTen[num % 10] : "");
        }
        if(num < 1000) {
        ...
        if(num < 1000000000000) {
            return numberToWords(num / 1000000000) + " Billion" + ((num % 1000000000 != 0) ? " " + numberToWords(num % 1000000) : "");
        }
    }
};
