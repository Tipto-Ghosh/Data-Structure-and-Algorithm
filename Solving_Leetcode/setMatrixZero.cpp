...
class Solution {
public:
    void setZeroRow(vector<vector<int>> &arr , int m , int n , int row) {
        for(int i  = 0; i < n; i++) {
            arr[row][i] = 0;
        }
    }
    void setZeroCol(vector<vector<int>> &arr , int m , int n , int col) {
        for(int i  = 0; i < m; i++) {
            arr[i][col] = 0;
        }
    }
    void setZeroes(vector<vector<int>>& arr) {
        ...
        for(int row : rowSet) {
            setZeroRow(arr , m , n , row);
        }
        for(int col : colSet) {
            setZeroCol(arr , m , n , col);
        }
...
int main() {
    ...
    vector<vi> arr = {{0,1,2,0} , {3,4,5,2} , {1,3,1,5}};
    Solution s;
    s.setZeroes(arr);
