...
public:
    int maximumSwap(int num) {
        ...
        int n = arr.size();
        int left = 0; 
        while(left < n && arr[left] == 9) {
            left++;
        }

        // now left == n or arr[left] < 9. So now find the largest digit from left + 1 to n - 1
        // Find the minimum digit from left and maxDigit from right
        ...
        if(swapLeft != -1) {
            swap(arr[swapLeft] , arr[swapRight]);
        }
