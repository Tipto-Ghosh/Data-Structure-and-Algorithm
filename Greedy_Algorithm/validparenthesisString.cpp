...
class Solution {
public:
	bool solve(int index , int n , string &s , int count , vector<vector<int>> &dp) {
		if(index > n || count < 0) {
			return false;
		}
		if(index == n) {
		...
		if(s[index] == '(') {
			ans1 = solve(index + 1 , n , s , count + 1 , dp);
		}
		else if(s[index] == ')') {
			ans2 = solve(index + 1 , n , s , count - 1 , dp);
		}
		else { // s[index] == '*'
            bool a = solve(index + 1 , n , s , count + 1 , dp); // taking as opening
            bool b = solve(index + 1 , n , s , count - 1 , dp); // taking as closing
            bool c = solve(index + 1 , n , s , count ,dp); // taking as empty
            ans3 = a || b || c;
		}
        dp[index][count] = ans1 || ans2 || ans3;
...
void let_him_cook() {
    ...
    cout << sol.checkValidString(s1) << endl;
    cout << sol.checkValidString(s2) << endl;
    cout << sol.checkValidString(s3) << endl;
    cout << sol.checkValidString(s4) << endl;
}
//--------------------------Main Here-------------------------------
int main() {
