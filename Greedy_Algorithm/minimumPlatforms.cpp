...
//         for(int i = 0; i < n; i++) {
//           arr[i] = {convertTime(arrival[i]) , convertTime(departure[i])};
//         }

//         // Sort based on the departure time
//         sort(all(arr) , cmp); 

//         // track the earliest departure time
//         priority_queue<int , vector<int> , greater<int>> minHeap;
//         int countPlatform = 0;

//         for(int i = 0; i < n; i++) {
//         	int arrTime = arr[i].first;
//         	int depTime = arr[i].second;

//         	// remove all the trains that have already ddeparted
//         	while(!minHeap.empty() && minHeap.top() <= arrTime) {
//         		minHeap.pop();
//         	}

//         	// add the curr_train
//         	minHeap.push(depTime);
//         	countPlatform = max(countPlatform , (int)minHeap.size());
//         }
