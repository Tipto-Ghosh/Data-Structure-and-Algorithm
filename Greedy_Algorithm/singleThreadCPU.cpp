...
public:
    vector<int> getOrder(vector<vector<int>>& tasks) {
        // add the index with all the tasks
        int n = tasks.size();
        for(int i = 0; i < n; i++) {
        	tasks[i].push_back(i);
        }
        ...
        // Now initialize the timer and add all the ele with this time
        lli timer = tasks[0][0]; // starting time
        int i = 0;
        vector<int> ans;
        while(i < n || !minHeap.empty()) {
            // put all the available tasks
            while(i < n && timer >= tasks[i][0]) {
            	minHeap.push({tasks[i][1] , tasks[i][2]}); // proccessing_time , index
            	i++;
            }
            ...
            if(minHeap.empty()) {
              ...
              auto t = minHeap.top();
              minHeap.pop(); 	
              ans.push_back(t.second); // Storing Index
              timer += t.first;
            }
        }
      return ans;  
