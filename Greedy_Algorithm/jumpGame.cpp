...
// class Solution {
// public:
// 	bool solve(int index , int n, vector<int>& arr , vector<int>& dp) {

// 		if(index >= n - 1) return true; // we have reached
// 		if(dp[index] != -1) return dp[index];
        
//         int maxJump = min(index + arr[index] , n - 1);

// 		for(int next = index + 1; next <= maxJump; next ++) {
// 			if(solve(next , n , arr, dp)) {
// 				dp[index] = true;
// 				return true;
// 			}
// 		}
...
public:
    bool canJump(vector<int>& arr) {
        ...
        for(int i = 0; i < n; i++) {
        	...
        	farestDis = max(farestDis , i + arr[i]);

        	if(farestDis >= n - 1) return true;
        } 

      return farestDis >= n - 1;       
    }
};
