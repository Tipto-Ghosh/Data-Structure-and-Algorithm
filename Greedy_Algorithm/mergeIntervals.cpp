...
public:
    vector<vector<int>> merge(vector<vector<int>>& intervals) {
        ...
        // sort all the intervals based on starting position
        sort(all(intervals));
        ...
        while(i < n) {
            // if overlap[currInterval starts before ending prev-->overlap]
            if(ans.back()[1] >= intervals[i][0]) {
               // dont need to update start cause already sorted 
