...
public:
    int rotationCount(long long R, long long D) {
        ...
        while(R != 0 || D != 0) {
            ...
            int antiCW = 10 - clockWise;

            ans += min(clockWise , antiCW); 
        }
      return ans;  
    }
};
...
void let_him_cook() {
   ...
   cout << s.rotationCount(98147 , 23985) << endl;
   cout << s.rotationCount(222 , 333) << endl;
   cout << s.rotationCount(2345 , 5432) << endl;
}
//--------------------------Main Here-------------------------------
int main() {
