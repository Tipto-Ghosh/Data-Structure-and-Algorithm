...
//         if(index >= n - 1) return 0; // we reached

//         if(dp[index] != -1) return dp[index];

//         int ans = INT_MAX;
//         // try every jump from 1 to curr_jump_value which is arr[index]
//         for(int countJump = 1; countJump <= arr[index]; countJump++) {
//             int subAns = solve(index + countJump , n , arr , dp);
//             if(subAns != INT_MAX) {
...
void let_him_cook() {
    Solution s;
    vector<int> arr1 = {2,3,1,14} , arr2 = {2,3,0,1,4};

    cout << s.jump(arr1) << endl;
    cout << s.jump(arr2) << endl;
}
//--------------------------Main Here-------------------------------
int main() {
