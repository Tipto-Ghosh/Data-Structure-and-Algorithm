...
bool cmp(Item &a , Item &b) {
	double aWeight = double(a.value) / a.weight;
	double bWeight = double(b.value) / b.weight;
...
class Solution {
  public:  
    double fractionalKnapsack(int w, Item arr[], int n) {
        ...
        // for(int i = 0; i < n; i++) {
        // 	cout << arr[i].value << " " << arr[i].weight << " " << arr[i].value / arr[i].weight << endl;
        // }
        ...
        for(int i = 0; i < n; i++) {
        	if(arr[i].weight <= w) {
              ...
              double perKgValue = double(arr[i].value) / arr[i].weight;
              ans += (w * perKgValue);
              break;
        	}
        }
...
void let_him_cook() {
   ...
   double ans = s.fractionalKnapsack(w1 , arr , n);
   cout << ans << endl;
}
//--------------------------Main Here-------------------------------
int main() {
