...
bool checking(vector<int> &boards, int timeAllowed , int numOfPainters) {
    int n = boards.size();
    int countPainters = 1; // Painter 1 is working
    int currTime = 0;

    for(int i = 0; i < n; i++) {
    	if(boards[i] > timeAllowed) return false;

    	if(currTime + boards[i] <= timeAllowed) {
           currTime += boards[i]; 
    	}
    	else {
    		countPainters ++; // switch the painter
    		currTime = boards[i];
    	}
    }
  return countPainters <= numOfPainters;    
}
...
int findLargestMinDistance(vector<int> &boards, int k) {
   ...
   while(minTime <= maxTime) {
   	 ...
   	 bool check = checking(boards , midTime , k);
...
void let_him_cook() {
    ...
    vector<int> arr2 = {24,90};

    cout << findLargestMinDistance(arr1 , 2) << endl;
}
//--------------------------Main Here-------------------------------
int main() {
