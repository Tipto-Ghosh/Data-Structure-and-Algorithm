...
// Step-1: make the heapify 
void maxHeapify(int index , vector<int> &arr , int size) {
	int largest = index;
	int left = 2 * index + 1;
	int right = 2 * index + 2;
...
// Step-2: Build the heap dunction
...
void let_him_cook() {
    vector<int> arr = {3,2,1,8,9,0,6,5,4,7};
    heapSort(arr);
