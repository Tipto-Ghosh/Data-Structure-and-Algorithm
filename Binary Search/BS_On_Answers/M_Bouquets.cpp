...
public:
    int minDays(vector<int>& bloomDay, int m, int k) {
        int flowerNeeded = m * k;
        int n = bloomDay.size();

        if(n < flowerNeeded) return -1; // we don't have enough flowers

        int maxDay = *max_element(all(bloomDay));
        ...
        while(left <= right) {
        	...
        	int numOfBouquets = 0 , numOfFlower = 0;
        	...
        	if(m > numOfBouquets) { // we need more flower so extend day
               left = mid + 1;
        	} 
        	else { // we already get a day but try to short the day
              ans = min(ans , mid);
              right = mid - 1;
        	}
        }
...
void let_him_cook() {
   ...
   vector<int> a2 = {7,7,7,7,12,7,7};

   cout << s.minDays(a , 3 , 1) << endl;
   cout << s.minDays(a , 3 , 2) << endl;
   cout << s.minDays(a2 , 2 , 3) << endl;  
}
//--------------------------Main Here-------------------------------
int main() {
