...
bool placeCow(int distance , int cowCount , vector<int> &arr) {
    // place the cow at 0-th index
    int count = 1;
    int lastPlacedCow = arr[0];
    int n = arr.size();
    
    for(int i = 1; i < n; i++) {
       if(arr[i] - lastPlacedCow >= distance) {
          count ++;
          lastPlacedCow = arr[i];
       } 
...
int aggressiveCows(vector<int> &arr , int k) {
   ...
   int right = arr[n-1] - arr[0]; // maximum distance that possible

   while(left <= right) {
      int mid = left + (right - left) / 2;
      // now check can we place all the cows with mid distance?
      bool check = placeCow(mid , k , arr);
      ...
      if(check) {
        ...
        right = mid - 1; // not possible so minimize the distance
      }
   }
   return ans;
}
...
void let_him_cook() {
   ...
   cout << aggressiveCows(arr2 , 2) << endl;
   cout << aggressiveCows(arr3 , 4) << endl;
}
//--------------------------Main Here-------------------------------
int main() {
