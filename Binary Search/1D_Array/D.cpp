...
public:
    bool canArrange(vector<int>& arr, int k) {
        ...
        for(auto &ele : map) {
            ...
            // if rem is 0 then we need same 0 rem value even times otherwise false
            if(rem == 0) {
            	if(count % 2 != 0) return false;
            }
            else {
            	int complement = k - rem;
            	if(map[complement] < count) return false; // don't have enough ele to make pair

            	// otherwise just take count number of complements
            	map[complement] -= count;
            }
        }
...
void let_him_cook() {
    ...
    // vector<int> arr2 = {-1,1,-2,2,-3,3,-4,4};
    // cout << s.canArrange(arr2 , 3) << endl;
