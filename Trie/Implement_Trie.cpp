...
struct Node {
  ...
  // is char is present or not
  bool isCurrCharExists(char c) {
...
public:
    ...
    void insert(string word) {
       ...
       for(int i = 0; i < n; i++) {
       	  ...
       	  if(!node->isCurrCharExists(word[i])) {
       	  	node->addChar(word[i]);
       	  }
       	  node = node->get(word[i]);
       }
       // now node is at last ending node
       node->isEnd = true;
    }
    ...
    bool search(string word) {
       ...
       for(char c : word) {
       	  if(node->isCurrCharExists(c) == false) {
       	  	return false;
       	  }
          node = node->get(c);
       }
       return node->isEnd; // true if the finding word is finished else false
    }
