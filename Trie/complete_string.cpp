...
public: 
    ...
    bool isAllPrefixExists(string &s) {
       Node* node = root;
...
string completeString(int n, vector<string> &arr){
    ...
    for(string &s : arr) {
        bool isComplete = trie.isAllPrefixExists(s);

        if(isComplete) {
            if(ans.length() == s.length()) {
              int lexxo = lexicographically(ans , s);
