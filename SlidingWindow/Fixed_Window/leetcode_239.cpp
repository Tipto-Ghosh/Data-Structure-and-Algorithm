...
public:
    ...
    vector<int> solve(vector<int> &arr , int k) {
        ...
        for(int right = 0; right < n; right++) {
            maxEle = max(maxEle , arr[right]);
            if(right - left + 1 > k) {
                // shrink the window and find the new max
                if(maxEle == arr[left]) {
                   // find the new max
                    left++;
                   maxEle = *max_element(arr.begin()+left , arr.begin()+right); 
                }
                else {
                  left++;
                }
...
int main() {
    ...
    vector<int> arr2 = {1 , -1};
    vector<int> ans2 = s.maxSlidingWindow(arr2 , 1); 
