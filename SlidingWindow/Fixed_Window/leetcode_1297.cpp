...
public:
    int maxFreq(string s, int maxLetters, int minSize, int maxSize) {
        ...
        for(int right = 0; right < n; right++) {
            // add occ of curr letter
            charOcc[s[right]] ++;

            // shrink the window if we exceed minSize
            if(right-left+1 > minSize) {
                charOcc[s[left]] --;
                if(charOcc[s[left]] == 0) {
                    charOcc.erase(s[left]);
                }
                left ++;
            }

            // if we have minSize window and unique chars don't cross the limit then add count
            if(right - left + 1 == minSize && charOcc.size() <= maxLetters) {
               // find that substring
               string sub = s.substr(left , minSize);
               subStringFreq[sub] ++;
               maxFreqCount = max(maxFreqCount , subStringFreq[sub]); 
            }
        }
      return maxFreqCount;    
    }
...
int main() {
    ...
    int ans = sol.maxFreq(s , 2 , 3 , 4);
    int ans2 = sol.maxFreq(s2 , 1 , 3 , 3);
    cout << ans << endl;
    cout << ans2 << endl;
