...
public:
    vector<vector<int>> spiralMatrix(int m, int n, ListNode* head) {
        ...
        while(top <= bottom && left <= right) {
        	// traverse the top row and col left to right
        	...
        	top++;
        	// now traverse right col and row top to bottom
        	...
        	right--;
        	if(!curr || top > bottom || left > right) break;
...
class Solution {
public:
    void decToBinary(int n , vector<int> &arr) {
        int index = 31;
        while(n != 0) {
           int lastDigit = n % 2;
           n /= 2;
