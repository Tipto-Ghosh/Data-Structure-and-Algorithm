...
public:
    int eraseOverlapIntervals(vector<vector<int>>& arr) {
        ...
        int start = arr[0][1] , end = arr[0][1];

        for(int i = 1; i < n; i++) {
          // if it's not an overlap then ignore it And if it is an overlap then take the smallest interval
...
int main() {
    ...
    vector<vector<int>> arr = {{0,2},{1,3},{2,4},{3,5},{4,6}};

    cout << s.eraseOverlapIntervals(arr) << endl;
