...
public:
    vector<ListNode*> splitListToParts(ListNode* head, int k) { 
        ...
        for(int i = 0; i < k; i++) {
        	// find number of node at i-th index
        	int currPartNum = eachPartSize;
        	if(extraPart > 0) {
        		currPartNum += 1;
        		extraPart --;
        	}
            
            // put the currHead in ans
            ans[i] = curr;

        	// move curr forwar by currpartNum nodes
        	for(int j = 0; j < currPartNum && curr != nullptr; j++) {
        		// if we are at last pos
        		if(j == currPartNum - 1) {
