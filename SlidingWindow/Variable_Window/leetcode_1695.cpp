...
public:
    int maximumUniqueSubarray(vector<int>& arr) {
        ...
        for(int right = 0; right < n; right++) {
        	if(map.find(arr[right]) != map.end()) { // means we already get this ele before
               ans = max(ans , currSum);

               int index = map[arr[right]];
               // move left pointer to the index next point 
               // remove all the elements from left till the index from the window[both inclusive]
...
int main() {
    ...
    vector<int> arr2 = {5,2,1,2,5,2,1,2,5};

    cout << s.maximumUniqueSubarray(arr) << " " << s.maximumUniqueSubarray(arr2) << endl;
    //  cout << s.maximumUniqueSubarray(arr); 
