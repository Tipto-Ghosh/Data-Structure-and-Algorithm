...
public:
    int subarraysWithKDistinct(vector<int>& arr, int k) {
        ...
        for(int right = 0; right < n; right++) {
            ...
            // if we exceed the window
            while(m.size() > k) {
                m[arr[leftNear]] --;

                if(m[arr[leftNear]] == 0) {
                    m.erase(arr[leftNear]);
                }
            ...
            // try to delete extra dublicate to make the window smaller by removing leftNear count
            while(m[arr[leftNear]] > 1) {
                m[arr[leftNear]]--;
                leftNear++;
            }
...
int main() {
   ...
   Solution s;
   vector<int> arr1 = {1,2,1,2,3} , arr2 = {1,2,1,3,4};

   cout << s.subarraysWithKDistinct(arr1 , 2) << endl;
   cout << s.subarraysWithKDistinct(arr2 , 3) << endl;
