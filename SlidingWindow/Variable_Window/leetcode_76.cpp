...
public:
    string minWindow(string s, string t) {
        ...
        pair<int , int> p = {-1,-1};
        int minLen = INT_MAX;
        int have = 0 , need = occT.size();
        int left = 0;

        for(int right = 0; right < n; right++) {
           char c = s[right];

           if(occT.find(c) != occT.end()) {
              occS[c] ++;
              if(occS[c] == occT[c]) {
                have += 1;
              }
           ...
           while(have == need) {
              ...
              // try to minimize the window len. Shift left forward
              char l = s[left];
              
              if(occT.find(l) != occT.end()) {
                occS[l] --;
                if(occS[l] < occT[l]) {
                    have--;
                }
      ...
      return s.substr(p.first , minLen);
    }
};
