...
public:
    int minimumCardPickup(vector<int>& arr) {
        int n = arr.size();
        int ans = INT_MAX;
        // store the ele and index
        unordered_map<int , int> map;

        for(int i = 0; i < n; i++) {
        	if(map.find(arr[i]) != map.end()) { // we saw it before
               int currWinLen = i - map[arr[i]] + 1;
               ans = min(ans , currWinLen);
        	}
        	map[arr[i]] = i;
        }       
...
int main() {
    ...
    Solution s;
    vector<int> arr = {3,4,2,3,4,7};
    vector<int> arr2 = {1,0,5,3};
    vector<int> arr3 = {95,11,8,65,5,86,30,27,30,73,15,91,30,7,37,26,55,76,60,43,36,85,47,96,6};

    int ans = s.minimumCardPickup(arr);
    int ans2 = s.minimumCardPickup(arr2);
    int ans3 = s.minimumCardPickup(arr3);
