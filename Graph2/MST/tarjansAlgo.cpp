...
void dfs(int node,int parent,int &time , vector<vector<int>>&adj , vector<bool>&vis , vector<int> &discTime , vector<int>&lowDiscTime , int &countBridge) {
	vis[node] = true;
	discTime[node] = lowDiscTime[node] = ++time;
	...
	for(int neiNode : adj[node]) {
		if(neiNode == parent) {
			...
			dfs(neiNode , node , time , adj , vis , discTime , lowDiscTime , countBridge);
		    lowDiscTime[node] = min(lowDiscTime[node] , lowDiscTime[neiNode]);

		    if(discTime[node] < lowDiscTime[neiNode]) {
		    	countBridge ++;
		    	cout << "[" << node << " , " << neiNode << "]" << endl;
		    }
		}
		else {
			lowDiscTime[node] = min(lowDiscTime[node] , discTime[neiNode]);
		}
	}
}
int count_bridge(int v , int e , vector<vector<int>> &edges) {
    ...
    int time = 0 , countBridge = 0;
    
    for(int i = 0; i < v; i++) {
    	if(!vis[i]) {
    		dfs(i , -1 , time , adj , vis , discTime , lowDiscTime , countBridge);
    	}
    }
...
void let_him_cook() {
    ...
    int countBridge = count_bridge(v , e , edges);
    cout << countBridge << endl; 
}
//--------------------------Main Here-------------------------------
int main() {
