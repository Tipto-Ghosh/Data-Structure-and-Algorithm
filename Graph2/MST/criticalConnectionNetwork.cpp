...
public:
	void dfs(int node , int parent , int &time , vector<vector<int>> &adj , vector<bool> &vis , vector<int>&discTime , vector<int> &low) {
		vis[node] = true;
		discTime[node] = low[node] = ++time;

		for(int neiNode : adj[node]) {
			if(neiNode == parent) continue;
			else if(!vis[neiNode]) {
				dfs(neiNode , node , time , adj , vis , discTime , low);
                low[node] = min(low[node] , low[neiNode]);
				if(discTime[node] < low[neiNode]) {
                   ans.push_back({node , neiNode});
				}
			} 
			else { // if visited
               low[node] = min(low[node] , discTime[neiNode]);
			}
		}
	}
    vector<vector<int>> criticalConnections(int n, vector<vector<int>>& connections) {
        ...
        vector<int> low(n , -1);
        int time = 0;
        dfs(0 , -1 , time , adj , vis , discTime , low);
      return ans;  
    }
};
...
void let_him_cook() {
    ...
    vector<vector<int>> ans = s.criticalConnections(n , conn);
    for(auto &ele : ans) {
