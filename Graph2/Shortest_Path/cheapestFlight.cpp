...
public:
    int findCheapestPrice(int n, vector<vector<int>>& flights, int src, int dest, int k) {
        ...
        dis[src] = 0;
        stops[src] = 0;
        minHeap.push(make_tuple(0 , src , 0)); // [cost , node , stops]
        ...
        while(!minHeap.empty()) {
        	...
        	int cost = get<0>(t);
        	int stop = get<2>(t);
            // if we reached dest then return cost
            if(node == dest) return cost;
            // if this path already cross k stops then no meaning to explore it . skip
        	if(stop > k) continue;
            
            // explore this path
            for(auto ele : adj[node]) {
            	int neiNode = ele.first;
            	int neiNodeCost = ele.second;
                int newCost = cost + neiNodeCost;

            	if(newCost < dis[neiNode] || stop + 1 < stops[neiNode]) {
            		dis[neiNode] = newCost;
            		stops[neiNode] = stop + 1;
            		minHeap.push(make_tuple(newCost , neiNode , stop + 1));
            	}
            }
        }
      return -1; // we can't reach dest     
    }  
};
...
void let_him_cook() {
    ...
    cout << s.findCheapestPrice(n , edges , 0 , 3 , k);
}
//--------------------------Main Here-------------------------------
int main() {
