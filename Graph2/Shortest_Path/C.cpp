...
class Solution {
    ll solve(int indexA , int indexB , vector<int>& a, vector<int>& b , vector<vector<ll>> &dp) {
        if(indexA < 0) return 0;
        if(indexB < indexA) return LLONG_MIN;

        if(dp[indexA][indexB] != LLONG_MIN) return dp[indexA][indexB];
        
        ll skip = 0  , take = 0;

        skip = solve(indexA , indexB - 1 , a , b , dp);
        take = solve(indexA - 1 , indexB - 1 , a , b , dp);
        
        if(take != LLONG_MIN) {
            take += (ll)a[indexA] * b[indexB];
        }

        dp[indexA][indexB] = max(take , skip);
