...
class Solution {
	vector<int> dijktra(int n , vector<vector<pair<int , int>>> &adj , int src) {
...
public:
    int findTheCity(int n, vector<vector<int>>& edges, int distanceThreshold) {
        ...
        for(int src = 0; src < n; src++) {
        	vector<int> distance = dijktra(n , adj , src);
        	int countCities = 0;
        	...
        	if(countCities < count || (countCities == count && src > ans)) {
        		count = countCities;
        		ans = src;
        	}
        }
