...
public:
    ...
    int minimumEffortPath(vector<vector<int>>& arr) {
        ...
        vector<vector<int>> effortTo(n , vector<int> (m , INT_MAX)); // track min effort
        ...
        while(!minHeap.empty()) {
            ...
            for(int i = 0; i < 4; i++) {
                ...
                int currEffort = max(effort , abs(arr[row][col] - arr[r][c]));

                if(currEffort < effortTo[r][c]) {
                    effortTo[r][c] = currEffort;
                    minHeap.push(make_pair(currEffort , make_pair(r , c)));
                }
            }
        }

      return -1; // never we reached here   
    }
};
...
int main() {
    ...
    Solution s;
    vector<vector<int>> arr1 = {{1,2,2} , {3,8,2} , {5,3,5}};

    cout << s.minimumEffortPath(arr1) << endl;
