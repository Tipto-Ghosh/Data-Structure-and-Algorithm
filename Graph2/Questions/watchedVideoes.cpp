...
public:
    vector<string> watchedVideosByFriends(vector<vector<string>>& watchedVideos, vector<vector<int>>& friends, int id, int level) {
        ...
        while(!q.empty()) {
            ...
            for(int neiNode : friends[node]) {
                if(!vis[neiNode]) {
                    q.push(neiNode);
                     levels[neiNode] = 1 + nodeLevel;
                }
            }
        }
        
        vector<int> desiredFriendsList;
        for(int i = 0; i < n; i++) {
            if(levels[i] == level) {
                desiredFriendsList.push_back(i);
            } 
        }

        unordered_map<string , int> m;
        for(int f : desiredFriendsList) {
        ...
        for(auto &ele : m) {
            minHeap.push(make_pair(ele.second , ele.first));
        }
...
void let_him_cook() {
    vector<vector<string>> wv = {{"A" , "B"} , {"C"} , {"B","C"} , {"D"}};
    vector<vector<int>> friends = {{1,2},{0,3},{0,3},{1,2}};
    int id = 0 , level = 1;

    Solution s;
    
    vector<string> ans = s.watchedVideosByFriends(wv , friends , id , level);
