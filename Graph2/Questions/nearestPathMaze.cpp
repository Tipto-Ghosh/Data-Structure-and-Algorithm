...
class Solution {
	...
	int dfs(int row , int col , int n , int m , vector<vector<int>> &maze , vector<vector<bool>> &vis) {
		if(row >= n || col >= m) return INT_MAX;
		// base case for reached case
		if((row == n-1 || col == m-1) && maze[row][col] == '.') {
            return 0;  
		}
...
public:
    int nearestExit(vector<vector<char>>& maze, vector<int>& entrance) {
        ...
        int ans = dfs(entrance[0] , entrance[1] , rows , cols , maze , vis);
    }
};
